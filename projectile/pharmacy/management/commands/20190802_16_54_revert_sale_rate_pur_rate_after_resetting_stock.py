...
def get_sales_price(stock):
    # Return the rate of last sale of a stock
    ...
    last_obj = get_filtered_stock_io(
        {
            ...
            'status': Status.FREEZE,
            'purchase__isnull': False,
            'purchase__purchase_type': PurchaseType.ORDER,
            'rate__gt': 0
        }
    )
    return get_rate(last_obj)
...
def populate_rates_of_stocks(store_point=None):
    ...
    for item in tqdm(stocks):
        ...
        sale_rate = get_sales_price(item)
        order_rate = get_order_price(item)
        item.sales_rate = sale_rate
        item.purchase_rate = purchase_rate
        item.order_rate = order_rate
        item.calculated_price = calculated_price
        item.save(update_fields=['sales_rate', 'purchase_rate', 'order_rate', 'calculated_price'])
        start_stock_signal()
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        logger.info("Fixing Sale Rate, Purchase Rate, Order Rate and Calculated Price of Stock")

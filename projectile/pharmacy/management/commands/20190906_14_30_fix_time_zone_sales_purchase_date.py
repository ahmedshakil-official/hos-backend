...
def populate_sales_data():
    ...
    sales_failed = []
    data = get_csv_data_from_temp_file('tmp/sales_info.csv')
    for item in tqdm(data):
        try:
            sale = Sales.objects.values('created_at').get(pk=item['sales_id'])
            sale_date = datetime.datetime.strptime(item['sale_date'], '%Y-%m-%d')
...
def populate_purchase_data():
    ...
    purchase_failed = []
    data = get_csv_data_from_temp_file('tmp/purchase_info.csv')
    for item in tqdm(data):
        try:
            sale = Purchase.objects.values('created_at').get(pk=item['purchase_id'])
            purchase_date = datetime.datetime.strptime(item['purchase_date'], '%Y-%m-%d')
            new_datetime = datetime.datetime.combine(purchase_date, sale['created_at'].time())
            new_datetime = timezone.make_aware(new_datetime, timezone.utc)

...
class PurchaseOrderCompletedSearchAPITest(OmisTestCase):
    url = reverse('pharmacy.product.purchase_order_completed_search')

    # def setUp(self):
    #     super(PurchaseOrderCompletedSearchAPITest, self).setUp()

    def test_purchase_order_completed_search_get(self):
        supplier = PersonOrganizationSupplierFactory(
            company_name='test',
            organization=self.admin_user.organization
        )

        purchase_order_completed = PurchaseFactory.create_batch(
            3,
            organization=self.admin_user.organization,
        ...
        # delete first entry
        purchase_order_completed[0].delete()
